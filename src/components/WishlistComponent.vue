<template>
  <div class="wishlist-wrapper font-sans text-neutral-200">
    ...
    <transition name="slide-right">
      ...
      >
        ...
        >
          ...
          <!-- MATCHED CART DESIGN FOR "SELECTED" TEXT -->
          <div
            class="text-xs font-bold text-rose-500 bg-rose-600/10 px-2 py-1 rounded uppercase tracking-wider"
          >
        ...
        <div class="flex-grow overflow-y-auto p-6 custom-scrollbar">
          ...
          >
            ...
            <p class="text-sm text-center mb-6">
              Hit the heart icon on any product to save it for later.
            </p>
            <button
              @click="$emit('close')"
          ...
          <div v-else class="space-y-4">
            ...
            >
                ...
                <div class="flex-grow flex flex-col justify-between h-full py-1">
                  <div>
                    <div class="flex justify-between items-start">
                      <h4
                        class="text-white font-bold text-sm leading-tight pr-2 cursor-pointer"
                        @click="toggleSelection(item.id)"
                      >
                        {{ item.name }}
                      </h4>
                      <button
                        @click="promptRemoveSingle(item.id)"
                    ...
                    <!-- DYNAMIC STOCK CHECK (Reads from JSON stock value) -->
                    <p
                      class="text-xs font-bold mt-1 uppercase tracking-wider"
                      :class="item.stock > 0 ? 'text-green-500' : 'text-neutral-500'"
                    >
              ...
              <!-- Action: Move to Cart (Only disabled if stock is exactly 0) -->
              <button
                @click="moveToCart(item)"
                :disabled="item.stock === 0"
                class="w-full mt-2 text-sm font-bold py-2.5 rounded transition-colors flex justify-center items-center gap-2 uppercase tracking-wider"
                :class="
                  item.stock > 0
                    ? 'bg-neutral-800 hover:bg-rose-600 text-white'
        ...
        <!-- Footer (ONLY VISIBLE IF THERE ARE ITEMS) -->
        <div
          v-if="wishlistItems.length > 0"
          class="border-t border-neutral-800 p-6 bg-neutral-950 text-center"
        >
          <p class="text-xs text-neutral-500 mb-4">Items saved here do not reserve inventory.</p>
          <button
            @click="$emit('close')"
    ...
    <!-- Confirmation Modal for Bulk Remove -->
    <transition name="fade">
        ...
        >
          ...
          <div class="p-6">
            <p class="text-sm text-neutral-400 mb-4">
              Are you sure you want to remove these items from your wishlist?
            </p>
    ...
    <!-- Local Toast for wishlist actions -->
    <ToastNotification
      :show="showToast"
...
// Ensures we can emit the 'add-to-cart' event so the Toast appears
...
// Local toast for the wishlist drawer (signin prompt + moved-to-cart)
...
const selectedItems = computed(() =>
  wishlistItems.value.filter((i) => selectedIds.value.includes(i.id)),
)
const allSelected = computed(
  () =>
    wishlistItems.value.length > 0 &&
    wishlistItems.value.every((i) => selectedIds.value.includes(i.id)),
)
...
function toggleSelectAll() {
  if (allSelected.value) {
    ...
    selectedIds.value = wishlistItems.value.map((i) => i.id)
  }
}
...
function confirmRemove() {
  selectedIds.value.forEach((id) => wishlistStore.removeFromWishlist(id))
  selectedIds.value = []
  confirmModalOpen.value = false
}

function promptRemoveSingle(id) {
  if (!id) return
  selectedIds.value = [id]
  confirmModalOpen.value = true
}

// Move to cart and trigger the global toast!
function moveToCart(item) {
  if (!item) return

  if (!authStore.isAuthenticated) {
    lastAddedItem.value = 'Sign in to move items to cart.'
    toastVariant.value = 'signin'
    showToast.value = true
    clearTimeout(toastTimeout)
    clearTimeout(redirectTimeout)
    toastTimeout = setTimeout(() => (showToast.value = false), 3000)
    redirectTimeout = setTimeout(() => router.push({ path: '/login' }), 900)
    return
  }

  // 1. Add it to the Pinia Cart Store
  cartStore.addToCart(item)

  // 2. Remove it from the Wishlist
  wishlistStore.removeFromWishlist(item.id)

  // 3. Emit the event so a parent can react if needed
